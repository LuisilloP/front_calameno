...
type CategoriasTableProps = {
  ...
  onToggleActive: (categoria: Categoria) => void;
  onDelete: (categoria: Categoria) => void;
};

const baseColumns: AdminTableColumn<Categoria>[] = [
  {
    key: "nombre",
    label: "Nombre",
    render: (item) => (
...
export const CategoriasTable = ({
  ...
  const columns: AdminTableColumn<Categoria>[] = baseColumns.map(
    (column) =>
      column.key === "acciones"
        ? {
            ...column,
  ...
  return (
    <AdminTable
      title="Categorias"
      description="Organiza los grupos de productos para reportes."
      data={data}
      columns={columns}
      isLoading={isLoading}
      total={total}
      ...
      emptyState={{
        ...
        description:
          "Crea categorias para clasificar productos y filtros.",
      }}
      accent="sky"
    />
  );
