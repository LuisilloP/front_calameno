...
const useProveedorState = () => {
  ...
  const [pendingToggle, setPendingToggle] = useState<Proveedor | null>(null);
...
const ProveedoresPageContent = () => {
  ...
  const handleError = (error: unknown) =>
    handleBusinessError(error, {
      duplicateMessage: "Ya existe un proveedor con ese nombre.",
      queryClient,
  ...
  const handleSubmit = async (values: EntityFormValues) => {
    try {
      if (formMode === "create") {
        ...
        pushToast({
          tone: "success",
          message: `Proveedor ${values.nombre} registrado.`,
        });
      } else if (selected) {
        ...
        pushToast({
          tone: "success",
          message: `Proveedor ${values.nombre} actualizado.`,
        });
      }
  ...
  const requestToggle = (item: Proveedor) => {
    if (item.activa) {
  ...
  const runToggle = async (item: Proveedor, close = false) => {
    try {
      await updateMutation.mutateAsync({
        id: item.id,
      ...
      pushToast({
        ...
        message: item.activa
          ? `Proveedor ${item.nombre} desactivado.`
          : `Proveedor ${item.nombre} activado.`,
      });
      if (close) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      pushToast({
        tone: "warning",
        message: `Proveedor ${pendingDelete.nombre} eliminado.`,
      });
  ...
  return (
    <AdminPageShell
      ...
      subtitle="Controla altas y bajas de proveedores."
      helper="Incluye activar/desactivar por fila"
    >
      {banner && (
      ...
      <AdminConfirmDialog
        ...
        description={
          pendingToggle?.activa
            ? "No podras seleccionarlo para nuevas recepciones."
            : "El proveedor vuelve a estar disponible."
        }
        confirmLabel={
          pendingToggle?.activa ? "Desactivar" : "Activar"
        }
      ...
      <AdminConfirmDialog
        ...
        description="Se eliminara por completo del catalogo."
